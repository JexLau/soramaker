...
console.log('开始生成封面...', allVideoList.length);
allVideoList.forEach((video) => {
  const videoPath = video.videoUrl;
  const coverName = videoPath.split('/').pop().split('.')[0];
  const outputPath = path.resolve(process.cwd(), `public/covers/${coverName}.jpg`);
  // 如果封面已经存在，则不再生成
  if (fs.existsSync(outputPath)) {
    console.log(`封面已存在: ${outputPath}`);
    return;
  }
